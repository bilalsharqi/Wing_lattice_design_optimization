...
def make_movie_frame(
    ...
    plot_edges_by_stress(ax_i, intact_state["nodes"], intact_state["edges"], intact_state["member_stress"], sigma_allow, cmap, norm, lw=1.3, alpha=0.95)
    ...
    if damaged_state["nodes"].size > 0 and damaged_state["edges"].size > 0:
        plot_edges_by_stress(ax_d, damaged_state["nodes"], damaged_state["edges"], damaged_state["member_stress"], sigma_allow, cmap, norm, lw=1.3, alpha=0.95)
...
def run_backend(backend_name, settings):
    ...
    wing = settings["wing"]
    lattice_type = settings["lattice_type"]
    E_modulus = settings["E_modulus"]
    density = settings["density"]
    sigma_allow = settings["sigma_allow"]
    u_tip_max = settings["u_tip_max"]
    cond_max = settings["cond_max"]
    a_min = settings["a_min"]
    a_max = settings["a_max"]
    a_init = settings["a_init"]
    total_lift = settings["total_lift"]
    n_iter = settings["n_iter"]
    prune_after_iter = settings["prune_after_iter"]
    damage_mode = settings["damage_mode"]
    ...
    with h5py.File(h5_path, "w") as h5:
        ...
        for it in range(n_iter):
            ...
            areas_after_sizing = simple_sizing_update(areas, res.member_stress, sigma_allow, a_min, a_max)
